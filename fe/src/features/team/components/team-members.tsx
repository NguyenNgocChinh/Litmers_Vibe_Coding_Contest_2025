  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200">
      ...
      <ul className="divide-y divide-gray-200">
        {members?.map((member) => (
          <li key={member.id} className="flex items-center justify-between p-6">
            <div className="flex items-center">
              {member.users?.avatar_url ? (
                ...
                <div className="flex items-center justify-center w-10 h-10 bg-gray-200 rounded-full text-gray-500 font-medium">
                  {member.users?.name?.[0] || '?'}
                </div>
              )}
              <div className="ml-4">
                <p className="text-sm font-medium text-gray-900">{member.users?.name || 'Unknown'}</p>
      ...
      {isInviteModalOpen && (
        <InviteMemberModal teamId={teamId} onClose={() => setIsInviteModalOpen(false)} />
      )}
    </div>
  );
}
